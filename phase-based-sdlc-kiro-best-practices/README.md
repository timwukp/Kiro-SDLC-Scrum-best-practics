...
**Key principle:** Deterministic enforcement uses Shell Command (free, 100% reliable). LLM-reasoning checks use Agent Prompt (costs credits). Only Pre Tool Use hooks can block operations.
...
5. **Pre Tool Use blocks; Post Tool Use advises** — Only Pre Tool Use + non-zero exit code can block operations
